...
######################################################
    ...
    #####################################################
        ...
        # 806 ppr
        self.ticks_per_meter = rospy.get_param('ticks_meter', 3665)
        ...
        if (self.wheel_latest == self.wheel_prev):
            ...
            if abs(cur_vel) < self.vel_threshold:
                ...
                if abs(cur_vel) < self.vel:
                    ...
                    # we know we're slower than what we're currently publishing
                    # as a velocity
            ...
            self.wheel_prev = self.wheel_latest
            self.then = rospy.Time.now()
        # input for PID (feedback of close loop)
    ...
    #####################################################
        ...
        # rospy.loginfo("i = i + (e * dt): %0.3f = %0.3f + (%0.3f * %0.3f)" %
        # (self.integral, self.integral, self.error, pid_dt))
        ...
        rospy.logdebug("vel:%0.2f tar:%0.2f err:%0.2f int:%0.2f der:%0.2f ## motor:%d " % (self.vel, self.target, self.error, self.integral, self.derivative, self.motor))
...
#        rospy.logdebug("-D- %s wheelCallback msg.data= %0.3f wheel_latest =
#        %0.3f mult=%0.3f" % (self.nodename, enc, self.wheel_latest,
#        self.wheel_mult))
